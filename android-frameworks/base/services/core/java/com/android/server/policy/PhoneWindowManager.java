...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public long interceptKeyBeforeDispatching(IBinder focusedToken, KeyEvent event,
        ...
        switch(keyCode) {
            ...
            case KeyEvent.KEYCODE_F6:
                if (down) {
                    doStartApplicationWithPackageName("com.google.android.youtube.tv");
                    return key_consumed;
                }
                break;
            case KeyEvent.KEYCODE_F7:
                if (down) {
                    doStartApplicationWithPackageName("com.netflix.mediaclient");
                    return key_consumed;
                }
                break;
            case KeyEvent.KEYCODE_F8:
                if (down) {
                    doStartApplicationWithPackageName("com.android.vending");
                    return key_consumed;
                }
                break;
        }
    ...
    private class MultiHdmiVideoExtconUEventObserver extends ExtconStateObserver<Boolean> {
        ...
        private boolean init(String name) {
            ...
            try {
                ...
                Slog.e(TAG, "Error reading " + mHdmi.getStatePath()
                       + " while attempting to determine initial state", e);
            }
    ...
    private void doStartApplicationWithPackageName(String packagename) {
        ...
        // 閸掓稑缂撴稉鈧稉顏嗚閸掝偂璐烠ATEGORY_LAUNCHER閻ㄥ嫯顕氶崠鍛倳閻ㄥ嚘ntent
        Intent resolveIntent = new Intent(Intent.ACTION_MAIN, null);
        resolveIntent.addCategory(Intent.CATEGORY_LAUNCHER);
        resolveIntent.setPackage(packageinfo.packageName);

        // 闁俺绻僩etPackageManager()閻ㄥ墑ueryIntentActivities閺傝纭堕柆宥呭坊
        List<ResolveInfo> resolveinfoList = mContext.getPackageManager().queryIntentActivities(resolveIntent, 0);

        ResolveInfo resolveinfo = resolveinfoList.iterator().next();
        if (resolveinfo != null) {
            // packagename = 閸欏倹鏆焢ackname
            String packageName = resolveinfo.activityInfo.packageName;
            // 鏉╂瑤閲滅亸杈ㄦЦ閹存垳婊戠憰浣瑰閻ㄥ嫯顕欰PP閻ㄥ嚛AUNCHER閻ㄥ嚈ctivity[缂佸嫮绮愯ぐ銏犵础閿涙ackagename.mainActivityname]
            String className = resolveinfo.activityInfo.name;
            // LAUNCHER Intent
            Intent intent = new Intent(Intent.ACTION_MAIN);
            intent.addCategory(Intent.CATEGORY_LAUNCHER);

            // 鐠佸墽鐤咰omponentName閸欏倹鏆?:packagename閸欏倹鏆?:MainActivity鐠侯垰绶?+            
    ...
    private void startTvSettings() {

        Log.e(TAG, "usloy start tv settings!");

        Intent intent_tv_settings = new Intent();
        ComponentName componentname_tv_settings = new ComponentName("com.android.tv.settings",
                "com.android.tv.settings.MainSettings");
        intent_tv_settings.setComponent(componentname_tv_settings);
        intent_tv_settings.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent_tv_settings.setAction("android.intent.action.MAIN");
        // mContext.startActivity(intent_tv_settings);
        startActivityAsUser(intent_tv_settings, UserHandle.CURRENT_OR_SELF);

    }

    private void startTvApp() {
        // Intent intent = new Intent();
        // intent.setComponent(ComponentName.unflattenFromString("com.droidlogic.tvsource/com.droidlogic.tvsource.DroidLogicTv"));
        // mContext.startActivity(intent);
        
        try {
            Intent intent = new Intent();
            intent.setComponent(new ComponentName("com.hkw.simplelauncher", "com.hkw.simplelauncher.HomeScreen"));
            startActivityAsUser(intent, UserHandle.CURRENT_OR_SELF);
        } catch (Exception e) {
        }

        Log.e(TAG, "usloy start tv app key down!");
    }

    private void startKodiApp() {

        Log.e(TAG, "usloy start kodi app!");

        doStartApplicationWithPackageName("org.xbmc.kodi");
    }
    ...
    private void openBrower() {
        String url = SystemProperties.get("help.url", "www.google.com");
        Uri uri = Uri.parse(url);
        Intent intent = new Intent(Intent.ACTION_VIEW, uri);
        if (intent.resolveActivity(mContext.getPackageManager()) != null) {
            Log.e(TAG, "usloy start browser with: " + url);
            // mContext.startActivity(intent);
            startActivityAsUser(intent, UserHandle.CURRENT_OR_SELF);
        }
    }
