        ...
        : mAttributionSource(attributionSource)
{
    ...
    mFd655PlayFlag = false;
}
void MediaPlayer::writeSys(const char *path, const char *val){
    int fd;
...
{
    ALOGV("disconnect");
    writeSys("/sys/class/fd655/vplay_flag", "0");
    writeSys("/sys/class/fd655/vpause_flag", "0");
    sp<IMediaPlayer> p;
    {
...
//DTS
               if(mFd655PlayFlag){
                       writeSys("/sys/class/fd655/vplay_flag", "1");
                       writeSys("/sys/class/fd655/vpause_flag", "0");
                       mFd655PlayFlag =false;
               }
//DTS
...
status_t MediaPlayer::pause()
{
    ...
    if ((mPlayer != 0) && (mCurrentState & MEDIA_PLAYER_STARTED)) {
        ...
        if (ret != NO_ERROR) {
            ...
            writeSys("/sys/class/fd655/vplay_flag", "0");
            writeSys("/sys/class/fd655/vpause_flag", "1");
                       mFd655PlayFlag = true;
        }
        return ret;
    }
