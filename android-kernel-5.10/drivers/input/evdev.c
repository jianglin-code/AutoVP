...
static int evdev_ns_track_client(struct evdev_client *client)
{
	...
	client->is_always_active = false;//is_always_active_evdev(client);
			 ...
			 const struct input_value *vals, unsigned int count)
{
	...
	struct evdev_client *client;
	struct evdev_client *child = NULL;
	...
	if (client)
		...
		list_for_each_entry_rcu(child, &evdev->client_list, node){
			if(child->evdev_ns == client->evdev_ns)
				evdev_pass_values(child, vals, count, ev_time);
		}
	else
			   ...
			   size_t count, loff_t *ppos)
{
	...
	while (retval + input_event_size() <= count) {
		...
		if (!evdev_client_is_active(client))
			continue;
#endif
		pr_info("evdev_write: %s , event.type=%d , event.code=%d , event.value=%d \n", evdev->handle.dev->name, event.type, event.code, event.value);
		evdev_grab(evdev, client);
